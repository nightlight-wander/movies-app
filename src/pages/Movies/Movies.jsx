  ...
  useEffect(() => {
    (async () => {
      const { data } = await axios.get(
        `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.REACT_APP_KEY}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${page}&with_genres=${genreIdsString}`
      );
  ...
  return (
    <>
      ...
      <div className="changePage flex-center">
        <button className="page-index" onClick={() => prevPageHandler()}>
          &lt;
        </button>
        <Pages
        ...
        >
          {(index) => (
            <button
              className={`page-index ${page === index ? "active" : null}`}
              key={index}
              onClick={pageHandler}
            >
              {index}
        ...
        <button className="page-index" onClick={() => nextPageHandler()}>
          &gt;
        </button>
      </div>
    </>
